...
(defun update-game-state (index)
  (if (eq *current-player* *p1*)
      ...
      (when (and (> index *p1-position*) (<= (- index *p1-position*) 2))
        ...
        (modify-button-for-exp-window (nth *p1-position* *spaces*) :color 'white :text "")
        ;; new space is white with a 1
        (modify-button-for-exp-window (nth index *spaces*) :color 'white :text "1")
         ...
         (if (>= index *p2-position*) 
             (progn
               ...
               (modify-button-for-exp-window (nth index *spaces*) :color 'green))
          
          (progn ;; update the p1 position and make p2 the current player
            
            (modify-button-for-exp-window (nth *p2-position* *spaces*) :color 'blue)
    ...
    (when (and (< index *p2-position*) (<= (- *p2-position* index) 2))
       ...
       ;; update the two buttons
        ...
        ;; old space is now white and blank
        (modify-button-for-exp-window (nth *p2-position* *spaces*) :color 'white :text "")
        ;; new space is white with a 1
        (modify-button-for-exp-window (nth index *spaces*) :color 'white :text "2")
      ...
      (if (<= index *p1-position*) 
        ...
        (progn ;; update the p2 positon and make p1 the current player
          
          (modify-button-for-exp-window (nth *p1-position* *spaces*) :color 'red)
