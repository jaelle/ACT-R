      ...
      (when (and (> index *p1-position*) (<= (- index *p1-position*) 2))
        ...
        (if (>= index *p2-position*)
            ...
            (progn ;; player 1 wins
              ...
              (with-model-eval *p1*
                (modify-button-for-exp-window (nth *p1-position* *spaces-1*) :color 'white :text "")
                (modify-button-for-exp-window (nth index *spaces-1*) :color 'green :text "1")
                (proc-display))
              
              ;; update player2's board which is opposite
              ;; and have the model process it
              (with-model-eval *p2*
                (modify-button-for-exp-window (nth (- 5 *p1-position*) *spaces-2*) :color 'white :text "")
                (modify-button-for-exp-window (nth (- 5 index) *spaces-2*) :color 'green :text "2")
                (proc-display)))
          ...
          (progn  ;; non-winning player 1 move
            ...
            (with-model-eval *p1*
              (modify-button-for-exp-window (nth *p1-position* *spaces-1*) :color 'white :text "")
              (modify-button-for-exp-window (nth index *spaces-1*) :color 'white :text "1")
              (proc-display))
            ...
            (with-model-eval *p2*
              (modify-button-for-exp-window (nth (- 5 *p1-position*) *spaces-2*) :color 'white :text "")
              (modify-button-for-exp-window (nth (- 5 index) *spaces-2*) :color 'white :text "2")
              
              ;; it's now player2's turn so make the button red
              
              (modify-button-for-exp-window (nth (- 5 *p2-position*) *spaces-2*) :color 'red)
              (proc-display))
    ...
    (when (and (< index *p2-position*) (<= (- *p2-position* index) 2))
      ...
      (if (<= index *p1-position*)
          (progn 
            ...
            (with-model-eval *p1*
              (modify-button-for-exp-window (nth *p2-position* *spaces-1*) :color 'white :text "")
              (modify-button-for-exp-window (nth index *spaces-1*) :color 'green :text "2")
              (proc-display))
            
            (with-model-eval *p2*
              (modify-button-for-exp-window (nth (- 5 *p2-position*) *spaces-2*) :color 'white :text "")
              (modify-button-for-exp-window (nth (- 5 index) *spaces-2*) :color 'green :text "1")
              (proc-display)))
        (progn 
          (with-model-eval *p1*
            (modify-button-for-exp-window (nth *p2-position* *spaces-1*) :color 'white :text "")
            (modify-button-for-exp-window (nth index *spaces-1*) :color 'white :text "2")
            
            (modify-button-for-exp-window (nth *p1-position* *spaces-1*) :color 'red :text "1")
            (proc-display))
          
          (with-model-eval *p2*
            (modify-button-for-exp-window (nth (- 5 *p2-position*) *spaces-2*) :color 'white :text "")
            (modify-button-for-exp-window (nth (- 5 index) *spaces-2*) :color 'white :text "1")
            (proc-display))
